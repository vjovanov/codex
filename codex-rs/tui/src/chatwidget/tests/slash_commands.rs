...
#[tokio::test]
async fn queued_clear_after_active_turn_replays_following_prompts() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.thread_id = Some(ThreadId::new());
    handle_turn_started(&mut chat, "turn-1");

    queue_composer_text_with_tab(&mut chat, "/clear");
    queue_composer_text_with_tab(&mut chat, "first after clear");
    queue_composer_text_with_tab(&mut chat, "second after clear");

    // Finishing the turn dequeues the `/clear`, which requests a fresh UI and
    // stops draining. The prompts queued after it must stay in the queue so the
    // `ClearUi` handler can preserve them.
    complete_turn_with_message(&mut chat, "turn-1", Some("done"));

    let clear_events = std::iter::from_fn(|| rx.try_recv().ok())
        .filter(|event| matches!(event, AppEvent::ClearUi))
        .count();
    assert_eq!(
        clear_events, 1,
        "queued /clear should request a single UI clear"
    );
    assert_eq!(
        chat.queued_user_message_texts(),
        vec![
            "first after clear".to_string(),
            "second after clear".to_string()
        ]
    );

    // The `ClearUi` handler snapshots the remaining queue, builds a fresh
    // thread, and restores the snapshot into it.
    let input_state = chat
        .capture_pending_input_for_clear()
        .expect("prompts queued after /clear should be preserved");
    let (mut restored_chat, mut restored_rx, mut restored_op_rx) =
        make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert_eq!(
        restored_chat.queued_user_message_texts(),
        vec![
            "first after clear".to_string(),
            "second after clear".to_string()
        ]
    ...
    // The first preserved prompt runs immediately in the fresh thread; the
    ...
    match next_submit_op(&mut restored_op_rx) {
        Op::UserTurn { items, .. } => assert_eq!(
            items,
            vec![UserInput::Text {
                text: "first after clear".to_string(),
                text_elements: Vec::new(),
            }]
        ),
        other => panic!("expected first preserved prompt to start a user turn, got {other:?}"),
    }
    assert_eq!(
        restored_chat.queued_user_message_texts(),
        vec!["second after clear".to_string()]
    );
    let events = std::iter::from_fn(|| restored_rx.try_recv().ok()).collect::<Vec<_>>();
    assert!(
        !events
            .iter()
            .any(|event| matches!(event, AppEvent::ClearUi)),
        "replaying plain prompts must not request another clear; events: {events:?}"
    );
}

#[tokio::test]
async fn queued_clear_preserves_nested_clear_boundary() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.thread_id = Some(ThreadId::new());
    handle_turn_started(&mut chat, "turn-1");

    queue_composer_text_with_tab(&mut chat, "/clear");
    queue_composer_text_with_tab(&mut chat, "before second clear");
    queue_composer_text_with_tab(&mut chat, "/clear");
    queue_composer_text_with_tab(&mut chat, "after second clear");

    complete_turn_with_message(&mut chat, "turn-1", Some("done"));

    // Only the first `/clear` is consumed. The later `/clear` is still a queue
    // boundary and must survive so it can open another fresh thread for the
    // prompts after it instead of being silently flattened away.
    assert_eq!(
        chat.queued_user_message_texts(),
        vec![
            "before second clear".to_string(),
            "/clear".to_string(),
            "after second clear".to_string()
        ]
    );

    let input_state = chat
        .capture_pending_input_for_clear()
        .expect("queued input after /clear should be preserved");
    ...
    // The prompt before the nested boundary runs first; the nested `/clear` and
    // the prompt after it remain queued to drive the next fresh thread.
    restored_chat.maybe_send_next_queued_input();
    match next_submit_op(&mut restored_op_rx) {
        Op::UserTurn { items, .. } => assert_eq!(
            items,
            vec![UserInput::Text {
                text: "before second clear".to_string(),
                text_elements: Vec::new(),
            }]
        ),
        other => panic!("expected prompt before nested /clear to start a user turn, got {other:?}"),
    }
    assert_eq!(
        restored_chat.queued_user_message_texts(),
        vec!["/clear".to_string(), "after second clear".to_string()]
    );
}
