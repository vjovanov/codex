...
impl ChatWidget {
    ...
    /// Snapshot queued follow-up input so it can be replayed in the fresh
    /// thread created by a queued `/clear`.
    ///
    /// This must be called from the `ClearUi` handler, i.e. *after* the
    /// triggering `/clear` has already been dequeued by
    /// [`Self::maybe_send_next_queued_input`]. The remaining queue therefore
    /// contains only the input queued *after* that `/clear`. Any further queued
    /// `/clear` is preserved on purpose: once replayed it re-triggers `ClearUi`
    /// and opens another fresh thread for the prompts that follow it, so
    /// `/clear` keeps working as a queue boundary even when several are queued.
    ///
    /// The live composer draft and pending steers are intentionally dropped:
    /// the draft stays recallable from history, and steers only ever apply to
    /// the turn that just ended.
    pub(crate) fn capture_pending_input_for_clear(&self) -> Option<ThreadInputState> {
        if !self.has_queued_follow_up_messages() {
            return None;
        }

        let mut input_state = self.capture_thread_input_state()?;
        input_state.composer = None;
